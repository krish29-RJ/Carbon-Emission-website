...
import { Check, X, Target, Zap, Apple, Car, ShoppingBag, Award } from 'lucide-react';
    ...
    <div className="bg-slate-900/60 rounded-xl border border-white/8 p-5 hover:border-white/15 hover:bg-slate-800/60 transition-all duration-200">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full opacity-90 ${colors.badge}`}>
            {goal.category}
          </span>
          <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full opacity-90 ${difficultyColors[goal.difficulty]}`}>
            {goal.difficulty}
          </span>
        </div>
        <div className={`w-2.5 h-2.5 rounded-full ${isCompleted ? 'bg-emerald-500' : colors.dot}`} />
      </div>

      <h4 className={`text-base font-semibold mb-1.5 ${isCompleted ? 'text-slate-500 line-through' : 'text-white'}`}>
        {goal.title}
      </h4>
      {goal.description && (
        ...
        {isCompleted ? (
          <div className="flex items-center gap-1.5 text-emerald-400">
            ...
            <span className="text-sm font-medium">
              {goal.completed_at ? new Date(goal.completed_at).toLocaleDateString() : 'Done'}
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
