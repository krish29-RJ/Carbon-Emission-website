...
const EcoWorld = memo(function EcoWorld({
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full overflow-hidden rounded-2xl glass-card border border-white/10 select-none"
    >
      <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-400 mt-0.5">
            Total Impact: <span className="font-bold text-white">{total}</span>{" "}
            kg CO₂e
          </p>
        </div>
      </div>
      ...
      {interactive && !tooltip && (
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-center gap-1 text-[10px] text-slate-400 pointer-events-none">
          ...
          <span>
            Click glowing nodes or regions to explore environmental impact
            details.
          </span>
        </div>
      )}
