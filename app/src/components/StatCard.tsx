...
export default function StatCard({ icon, iconBg, label, value, unit, trend, trendLabel, subtext }: Props) {
    ...
    <div className="glass-card rounded-2xl p-5 shadow-sm hover:shadow-lg hover:border-emerald-500/20 hover:translate-y-[-2px] transition-all duration-300">
      ...
      {trend !== undefined && (
        <div className="flex items-center gap-1.5">
          <div className={`flex items-center gap-0.5 ${trendPositive ? 'text-emerald-400' : trendNeutral ? 'text-slate-400' : 'text-red-400'}`}>
            {trendPositive ? <TrendingDown className="w-3.5 h-3.5" /> : trendNeutral ? <Minus className="w-3.5 h-3.5" /> : <TrendingUp className="w-3.5 h-3.5" />}
            <span className="text-xs font-semibold">{Math.abs(trend)}%</span>
          </div>
          <span className="text-[10px] text-slate-500">{trendLabel || 'vs previous'}</span>
        </div>
      )}
