...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string }) {
  ...
  return (
    <div className="glass-card rounded-xl px-4 py-3 border border-white/10 shadow-2xl">
      <p className="text-[10px] text-slate-400 mb-1">{label ? format(parseISO(label), 'MMM d, yyyy') : ''}</p>
      <p className="text-lg font-bold text-emerald-400">{payload[0].value} kg</p>
      <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">CO₂e/month</p>
    </div>
  );
}
  ...
  if (!data || data.length === 0) {
    return (
      <div className="glass-card rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-xs text-slate-400 mb-4 font-medium">Your progress over time</p>
        <EmptyState
