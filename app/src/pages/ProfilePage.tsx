...
import {
  MapPin, Home, Mail, Edit3, Check, X,
  Leaf, Target, Award, BarChart3, Calendar,
  TrendingDown, TrendingUp, Calculator, ChevronRight,
} from 'lucide-react';
...
const avatarPresets = [
  ...
  { name: 'Eco Leaf', url: 'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?w=150&auto=format&fit=crop&q=60' },
  { name: 'Sunlight', url: 'https://images.unsplash.com/photo-1504608524841-42fe6f032b4b?w=150&auto=format&fit=crop&q=60' },
  { name: 'Ocean Wave', url: 'https://images.unsplash.com/photo-1505118380757-91f5f5632de0?w=150&auto=format&fit=crop&q=60' },
  { name: 'Mountain', url: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=150&auto=format&fit=crop&q=60' },
  { name: 'Blossom', url: 'https://images.unsplash.com/photo-1522748906645-95d8adfd52c7?w=150&auto=format&fit=crop&q=60' },
];

const bannerPresets = [
  { name: 'Default Gradient', url: '' },
  { name: 'Deep Forest', url: 'https://images.unsplash.com/photo-1448375240586-882707db888b?w=800&auto=format&fit=crop&q=60' },
  { name: 'Snowy Peak', url: 'https://images.unsplash.com/photo-1454496522488-7a8e488e8606?w=800&auto=format&fit=crop&q=60' },
  { name: 'Sunny Beach', url: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=800&auto=format&fit=crop&q=60' },
  { name: 'Desert Oasis', url: 'https://images.unsplash.com/photo-1509316975850-ff9c5deb0cd9?w=800&auto=format&fit=crop&q=60' },
  { name: 'Aurora Sky', url: 'https://images.unsplash.com/photo-1483347756197-71ef80e95f73?w=800&auto=format&fit=crop&q=60' },
];
...
export default function ProfilePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [reportsRes, allBadgesRes, userBadgesRes, goalsRes] = await Promise.all([
      supabase
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-20 pb-16 px-4 light text-foreground">
      <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex items-end justify-between -mt-10 mb-4">
              ...
              {!editing ? (
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={cancelEdit} className="border-slate-200">
            ...
            {!editing ? (
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-slate-500">
                  ...
                  {profile?.household_size && (
                    <span className="flex items-center gap-1.5">
                      <Home className="w-3.5 h-3.5" /> {profile.household_size} {profile.household_size === 1 ? 'person' : 'people'}
                    </span>
                  )}
                  <span className="flex items-center gap-1.5">
              ...
              <div className="space-y-4">
                <div className="grid sm:grid-cols-3 gap-4 mt-2">
                  ...
                  <div>
                    <Label htmlFor="editCity" className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                      City
                    </Label>
                    <Input
                      id="editCity"
                  ...
                  <div>
                    <Label htmlFor="editHousehold" className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                      Household Size
                    </Label>
                    <select
                      id="editHousehold"
                ...
                <div className="mt-6 space-y-6 border-t border-slate-100 pt-6">
                      ...
                      {avatarPresets.map((av) => (
                        <button
                          ...
                          className={`relative w-12 h-12 rounded-xl overflow-hidden border-2 transition-all ${
                            editAvatarUrl === av.url ? 'border-emerald-500 ring-2 ring-emerald-500/20 scale-105' : 'border-slate-200 opacity-70 hover:opacity-100'
                          }`}
                          title={av.name}
                        >
                          {av.url ? (
                      ...
                      {bannerPresets.map((bp) => (
                        <button
                          ...
                          className={`relative h-12 rounded-xl overflow-hidden border-2 transition-all ${
                            editBannerUrl === bp.url ? 'border-emerald-500 ring-2 ring-emerald-500/20 scale-105' : 'border-slate-200 opacity-70 hover:opacity-100'
                          }`}
                          title={bp.name}
                        >
                          {bp.url ? (
                  ...
                  <div>
                    ...
                    >
                      {showCustomUrls ? 'Hide Custom Image URL Inputs' : 'Use Custom Image URLs'}
                    </button>

                    {showCustomUrls && (
                      <div className="grid sm:grid-cols-2 gap-4 mt-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        <div>
                          <Label htmlFor="customAvatar" className="text-xs font-semibold text-slate-500">
                        ...
                        <div>
                          <Label htmlFor="customBanner" className="text-xs font-semibold text-slate-500">
          ...
          {[
            {
              ...
              iconBg: 'bg-sky-50',
              sub: reports.length === 0 ? 'None yet' : `Latest: ${latestReport?.total_co2e ?? '-'} kg`,
            },
            {
              label: 'Active Goals',
            ...
            },
            {
              ...
              iconBg: trend !== null && trend < 0 ? 'bg-emerald-50' : 'bg-red-50',
              sub: trend !== null ? (trend < 0 ? 'Improvement!' : 'Needs attention') : 'Need 2+ calculations',
              valueColor: trend !== null ? (trend < 0 ? 'text-emerald-700' : 'text-red-600') : 'text-slate-400',
            },
          ].map((stat) => (
            <div key={stat.label} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
              ...
              <p className={`text-2xl font-bold mb-0.5 ${stat.valueColor ?? 'text-slate-900'}`}>{stat.value}</p>
          ...
          >
            ...
            {badges.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-slate-400">Complete calculations to earn badges!</p>
                <Button size="sm" variant="outline" onClick={() => navigate('/calculator')}
              ...
              <div className="grid grid-cols-3 gap-3">
                {badges.map(badge => (
                  ...
                  >
                    ...
                    <p className={`text-[10px] font-semibold leading-tight ${badge.earned ? 'text-amber-700' : 'text-slate-400'}`}>
                      {badge.name}
                    </p>
                  </div>
                ))}
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-semibold text-slate-900">Recent Calculations</h3>
              </div>
              <button
            ...
            {reports.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-sm text-slate-500 mb-1 font-medium">No calculations yet</p>
                <p className="text-xs text-slate-400 mb-4">Track your carbon footprint to get started.</p>
                <Button onClick={() => navigate('/calculator')} className="bg-emerald-600 hover:bg-emerald-700 text-white">
              ...
              <div className="space-y-2">
                {reports.map((report, i) => {
                  ...
                  const change = prevReport
                    ? Math.round(((report.total_co2e - prevReport.total_co2e) / prevReport.total_co2e) * 100)
                    : null;
                  return (
                    <div
                      key={report.id}
                    ...
                    >
                      <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center shrink-0">
                        <span className="text-xs font-bold text-emerald-700">#{reports.length - i}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {(['transport', 'energy', 'food', 'lifestyle'] as const).map(cat => {
                            const val = cat === 'transport' ? report.transport_co2e
                              : cat === 'energy' ? report.energy_co2e
                              : cat === 'food' ? report.food_co2e
                              : report.lifestyle_co2e;
                            return (
                              <span key={cat} className={`text-[10px] font-medium px-1.5 py-0.5 rounded-md ${categoryColors[cat]}`}>
                                {val}
                              </span>
                            );
                          })}
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-base font-bold text-slate-900">{report.total_co2e}</p>
        ...
        >
          {[
            { label: 'Go to Dashboard', sub: 'View your full footprint overview', icon: <BarChart3 className="w-5 h-5" />, path: '/dashboard', color: 'text-emerald-700 bg-emerald-50 hover:bg-emerald-100' },
            { label: 'Browse Actions', sub: 'Accept challenges to reduce emissions', icon: <Target className="w-5 h-5" />, path: '/actions', color: 'text-sky-700 bg-sky-50 hover:bg-sky-100' },
            { label: 'Run Simulator', sub: 'Explore what-if scenarios', icon: <Leaf className="w-5 h-5" />, path: '/simulator', color: 'text-violet-700 bg-violet-50 hover:bg-violet-100' },
          ].map(action => (
            <button
              key={action.path}
