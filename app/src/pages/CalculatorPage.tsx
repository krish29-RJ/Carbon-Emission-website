  ...
  const handleSubmit = async () => {
    ...
    if (user && isSupabaseConfigured) {
      ...
      if (reportError) {
        ...
        toast.error(
          `Failed to save report: ${reportError.message}. Please run the setup script in supabase_schema.sql!`
        );
      } else if (report) {
        // Save activities
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-slate-400">
                {
                  ["Transport", "Home Energy", "Food", "Lifestyle", "Review"][
                    step
                  ]
                }
              </span>
...
function StepTransport({
  ...
  update: (k: keyof CalculatorInput, v: any) => void;
  onNext: () => void;
  custom: number;
}) {
  return (
    ...
    >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8 space-y-6">
        ...
        />
        <NumberField
          icon={<Bus className="w-5 h-5 text-sky-500" />}
          label="Bus kilometers per week"
          value={data.busKmPerWeek}
          onChange={v => update("busKmPerWeek", v)}
          placeholder="e.g., 50"
        />
        <NumberField
          icon={<Train className="w-5 h-5 text-sky-500" />}
          label="Train/metro kilometers per week"
          value={data.metroKmPerWeek}
          onChange={v => update("metroKmPerWeek", v)}
          placeholder="e.g., 30"
        />
        <NumberField
          ...
          onChange={v => update("flightHoursPerYear", v)}
          placeholder="e.g., 10"
        />
        ...
        <div className="flex items-start gap-3 p-4 bg-emerald-50 rounded-xl">
          ...
          <p className="text-sm text-emerald-700">
            Short flights have a much higher carbon cost per kilometer. One hour
            of flying equals about 90 kg CO2e.
          </p>
        </div>
    ...
    >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8 space-y-6">
        ...
        <NumberField
          ...
          onChange={v => update("acHoursPerDay", v)}
          placeholder="e.g., 4"
        />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Solar, wind, or green energy plan
              </p>
            </div>
          </div>
          <Switch
            checked={data.renewableEnergy}
            onCheckedChange={v => update("renewableEnergy", v)}
          />
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-emerald-50 rounded-xl">
          <Info className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
          <p className="text-sm text-emerald-700">
            The average household uses about 900 kWh per month. Check your
            electricity bill!
          </p>
        </div>
    ...
    >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {dietOptions.map(opt => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`w-5 h-5 ${data.dietType === opt.value ? "text-emerald-600" : "text-slate-400"}`}
                  />
                  <span
                    className={`text-sm font-medium ${data.dietType === opt.value ? "text-emerald-700" : "text-slate-600"}`}
                  >
                    {opt.label}
                  </span>
                </button>
        ...
        <NumberField
          ...
          label="Meat-containing meals per week"
        ...
        <NumberField
          ...
          value={data.foodWasteKgPerWeek}
          onChange={v => update("foodWasteKgPerWeek", v)}
          placeholder="e.g., 2"
        />
        ...
        <div className="flex items-start gap-3 p-4 bg-emerald-50 rounded-xl">
          <Info className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
          <p className="text-sm text-emerald-700">
            Reducing meat by just 2 meals per week saves ~36 kg CO2e per month.
          </p>
        </div>
    ...
    >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8 space-y-6">
        <NumberField
          ...
          onChange={v => update("shoppingOrdersPerMonth", v)}
          placeholder="e.g., 5"
        />
        <NumberField
          icon={<Truck className="w-5 h-5 text-violet-500" />}
          label="Food delivery orders per month"
          value={data.deliveryOrdersPerMonth}
          onChange={v => update("deliveryOrdersPerMonth", v)}
          placeholder="e.g., 3"
        />
        ...
        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
          ...
          <Switch
            checked={data.recyclesOften}
            onCheckedChange={v => update("recyclesOften", v)}
          />
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-emerald-50 rounded-xl">
          <Info className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
          <p className="text-sm text-emerald-700">
            Recycling can reduce your lifestyle footprint by up to 5%.
          </p>
        </div>
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        {
          label: "Electricity",
          value: `${data.electricityKwhPerMonth} kWh/month`,
        },
        { label: "AC usage", value: `${data.acHoursPerDay} hrs/day` },
        {
          label: "Renewable energy",
          value: data.renewableEnergy ? "Yes" : "No",
        },
    ...
    },
    {
      ...
      items: [
        {
          label: "Diet",
          value: data.dietType.charAt(0).toUpperCase() + data.dietType.slice(1),
        },
      ...
      items: [
        {
          ...
          value: `${data.shoppingOrdersPerMonth}/month`,
        },
        {
          label: "Delivery orders",
          value: `${data.deliveryOrdersPerMonth}/month`,
        },
    ...
    >
      <div className="text-center mb-8">
        ...
        <p className="text-slate-600">
          Double-check your answers before calculating.
        </p>
      </div>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <Button
          ...
          className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-5 rounded-xl"
        >
        ...
        <Button
          ...
          variant="outline"
          className="flex-1 py-5 rounded-xl border-emerald-600 text-emerald-700 hover:bg-emerald-50"
        >
