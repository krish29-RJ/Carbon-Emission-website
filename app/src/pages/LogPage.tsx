...
import {
  Car, Zap, Apple, ShoppingBag, Plane, AlertTriangle,
  CheckCircle, ChevronRight, Info, X, Leaf, Flame,
  Train, Bike, Home, Fish, Egg, TreePine, Coffee,
  Package, Smartphone, Shirt
} from 'lucide-react';
...
// ─── Nudge Messages ───────────────────────────────────────────────────────────
type NudgeData = { message: string; alternative: string; saving: number; icon: React.ElementType };

function getNudge(category: string, subtype: string, value: number): NudgeData | null {
  ...
  if (category === 'transport' && (subtype === 'flight_domestic' || subtype === 'flight_intl') && value > 100) {
    ...
    return {
      ...
      alternative: 'Consider rail alternatives or direct routes to reduce layover emissions.',
  ...
  if (category === 'food' && subtype === 'red_meat' && value > 0) {
    return {
      ...
      alternative: 'Swapping to chicken reduces food emissions by 75% for this meal.',
  ...
  if (category === 'energy' && subtype === 'ac_hour' && value > 4) {
    return {
      ...
      alternative: 'Setting your AC to 26°C instead of 22°C cuts energy use by ~18%.',
...
const CATEGORIES: Record<string, CategoryOption[]> = {
  transport: [
    { key: 'car_petrol', label: 'Petrol Car', icon: Car, factor: FACTORS.transport.car_petrol, unit: 'km', max: 200, step: 5, color: '#ef4444' },
    { key: 'car_electric', label: 'Electric Car', icon: Car, factor: FACTORS.transport.car_electric, unit: 'km', max: 200, step: 5, color: '#10b981' },
    { key: 'bus', label: 'Bus', icon: Car, factor: FACTORS.transport.bus, unit: 'km', max: 100, step: 5, color: '#f59e0b' },
    { key: 'train', label: 'Train', icon: Train, factor: FACTORS.transport.train, unit: 'km', max: 500, step: 10, color: '#06b6d4' },
    { key: 'metro', label: 'Metro', icon: Train, factor: FACTORS.transport.metro, unit: 'km', max: 60, step: 1, color: '#8b5cf6' },
    { key: 'flight_domestic', label: 'Domestic Flight', icon: Plane, factor: FACTORS.transport.flight_domestic, unit: 'km', max: 3000, step: 50, color: '#f97316' },
    { key: 'bike', label: 'Bicycle', icon: Bike, factor: 0, unit: 'km', max: 50, step: 1, color: '#34d399' },
  ],
  food: [
    { key: 'red_meat', label: 'Red Meat', icon: Apple, factor: FACTORS.food.red_meat, unit: '×100g', max: 10, step: 1, color: '#ef4444' },
    { key: 'chicken', label: 'Poultry', icon: Egg, factor: FACTORS.food.chicken, unit: '×100g', max: 10, step: 1, color: '#f59e0b' },
    { key: 'fish', label: 'Seafood', icon: Fish, factor: FACTORS.food.fish, unit: '×100g', max: 10, step: 1, color: '#06b6d4' },
    { key: 'dairy', label: 'Dairy', icon: Coffee, factor: FACTORS.food.dairy, unit: '×100g', max: 20, step: 1, color: '#a78bfa' },
    { key: 'vegetables', label: 'Vegetables', icon: Leaf, factor: FACTORS.food.vegetables, unit: '×100g', max: 30, step: 1, color: '#10b981' },
    { key: 'vegan', label: 'Vegan Meal', icon: TreePine, factor: FACTORS.food.vegan, unit: 'meals', max: 5, step: 1, color: '#34d399' },
  ],
  energy: [
    { key: 'electricity', label: 'Electricity', icon: Zap, factor: FACTORS.energy.electricity, unit: 'kWh', max: 50, step: 1, color: '#f59e0b' },
    { key: 'ac_hour', label: 'AC Usage', icon: Home, factor: FACTORS.energy.ac_hour, unit: 'hours', max: 24, step: 1, color: '#38bdf8' },
    { key: 'lpg_cylinder', label: 'LPG Cylinder', icon: Flame, factor: FACTORS.energy.lpg_cylinder, unit: 'cylinders', max: 5, step: 1, color: '#f97316' },
  ],
  shopping: [
    { key: 'clothing', label: 'Clothing Items', icon: Shirt, factor: FACTORS.shopping.clothing, unit: 'items', max: 10, step: 1, color: '#a78bfa' },
    { key: 'electronics', label: 'Electronics', icon: Smartphone, factor: FACTORS.shopping.electronics, unit: 'items', max: 5, step: 1, color: '#06b6d4' },
    { key: 'package_delivery', label: 'Deliveries', icon: Package, factor: FACTORS.shopping.package_delivery, unit: 'pkgs', max: 20, step: 1, color: '#f59e0b' },
    { key: 'coffee', label: 'Coffee Cups', icon: Coffee, factor: FACTORS.shopping.coffee, unit: 'cups', max: 10, step: 1, color: '#78350f' },
  ],
};
      ...
      try {
        const { data: report, error: reportError } = await supabase
          ...
          .insert({
            ...
            transport_co2e: parseFloat((categoryTotals.transport || 0).toFixed(2)),
            energy_co2e: parseFloat((categoryTotals.energy || 0).toFixed(2)),
            food_co2e: parseFloat((categoryTotals.food || 0).toFixed(2)),
            lifestyle_co2e: parseFloat((categoryTotals.shopping || 0).toFixed(2)),
            input_data: values,
          })
          .select()
          .single();
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Smart Activity Logger</h1>
              <p className="text-sm text-slate-400">Track your daily footprint with real-time carbon comparisons</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {TABS.map(tab => (
                <button
                  ...
                  style={activeTab === tab.id ? { background: `${tab.color}25`, border: `1px solid ${tab.color}40` } : {}}
                >
              ...
              {nudge && !nudgeDismissed && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-amber-200 font-medium">{nudge.message}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                      <nudge.icon className="w-3.5 h-3.5 text-emerald-400" />
                      <p className="text-xs text-emerald-400">{nudge.alternative}</p>
                    </div>
                    <p className="text-[10px] text-slate-500 mt-0.5">Potential saving: {nudge.saving.toFixed(1)} kg CO₂e</p>
                  </div>
                  <button onClick={() => setNudgeDismissed(true)} className="text-slate-500 hover:text-white transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </motion.div>
              )}
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1 mt-0.5">
                            <span className="text-xs text-slate-500">{val} {opt.unit}</span>
                            {kg > 0 && (
                      ...
                      <div className="flex items-center gap-1">
                        {[0, Math.round(opt.max * 0.25), Math.round(opt.max * 0.5), Math.round(opt.max * 0.75)].map(preset => (
                          <button
                            ...
                            style={val === preset ? { background: `${opt.color}30`, color: opt.color } : {}}
                          >
                    ...
                    <div className="relative">
                      <div className="relative h-2 rounded-full bg-slate-800">
                        <div
                          ...
                          style={{ width: `${pct}%`, background: `linear-gradient(to right, ${opt.color}80, ${opt.color})` }}
                        />
                      </div>
                      <input
                        type="range"
          ...
          <div className="space-y-4">
            ...
            >
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">Activity Summary</h3>
              ...
              <div className="text-center mb-5">
                <motion.p
                  ...
                  style={{ color: totalKg > 50 ? '#ef4444' : totalKg > 20 ? '#f59e0b' : '#10b981' }}
                >
                  {totalKg.toFixed(1)}
                </motion.p>
                <p className="text-sm text-slate-400 mt-1">kg CO₂e this entry</p>
                {totalKg > 0 && (
                  <p className="text-xs text-slate-500 mt-1 italic">{getComparison(totalKg)}</p>
                )}
              </div>
                    ...
                    <div key={tab.id}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-slate-300 capitalize">{tab.label}</span>
                        </div>
                        <span className="text-xs font-medium text-white">{catKg.toFixed(1)} kg</span>
                      </div>
              ...
              <div className="p-3 rounded-xl bg-slate-800/50 mb-4">
                <p className="text-[10px] text-slate-400 mb-1.5">India avg daily footprint</p>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-cyan-500 rounded-full"
                      style={{ width: `${Math.min((totalKg / 5) * 100, 100)}%` }}
                    />
                  </div>
              ...
              <AnimatePresence mode="wait">
                {logged ? (
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold text-emerald-400">Activity Logged!</span>
                  </motion.div>
                ) : (
                  <motion.div key="btn">
                    <Button
                    ...
                    <p className="text-[10px] text-slate-500 text-center mt-2">+50 XP for logging today</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
