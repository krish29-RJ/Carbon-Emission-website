...
import { Trophy, TrendingDown, TrendingUp, Users, Target, Calculator, Star } from 'lucide-react';
    ...
    try {
      ...
      (goals || []).forEach(g => {
        if (g.status === 'completed') {
          if (periodStart && (!g.completed_at || new Date(g.completed_at) < periodStart)) return;
      ...
      const leaderboard: LeaderboardEntry[] = profiles
        .map(p => {
          ...
          return {
            ...
            first_footprint: previous ? Math.round(previous * 100) / 100 : Math.round(latest * 100) / 100,
        ...
        .filter((e): e is LeaderboardEntry => e !== null && e.latest_footprint > 0);
    ...
    <div className="min-h-screen bg-slate-50 pt-20 pb-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Community Leaderboard</h1>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white text-sm ${
                  ...
                  {currentUserRank ? (
                    currentUserRank <= 3 ? rankMedals[currentUserRank - 1] : `#${currentUserRank}`
                  ) : '?'}
                </div>
                <div>
                  {currentUserEntry ? (
                    <>
                      ...
                      <p className="text-xs text-emerald-600 mt-0.5">
                        ...
                        {currentUserEntry.latest_footprint} kg CO₂e · {currentUserEntry.completed_goals} goals completed
                      </p>
                    </>
                  ) : (
                    <>
                      <p className="text-sm font-semibold text-amber-800">You are not on the board yet</p>
        ...
        {entries.length === 0 ? (
          <EmptyState
            ...
            title={isSupabaseConfigured ? 'Be the first!' : 'Leaderboard unavailable'}
            description={
              isSupabaseConfigured
                ? 'Complete your first calculation to appear on the leaderboard.'
                : 'Connect Supabase to load community rankings.'
            }
            action={isSupabaseConfigured ? { label: 'Start Calculator', onClick: () => navigate('/calculator') } : undefined}
          />
        ) : (
            ...
            {entries.map((entry, i) => (
              <div
                ...
                className={`grid grid-cols-12 gap-2 px-4 py-4 border-b border-slate-50 last:border-0 items-center hover:bg-slate-50/50 transition-colors ${
                  entry.isCurrentUser ? 'bg-emerald-50/60 border-l-4 border-l-emerald-400' : ''
                }`}
              >
                {/* Rank */}
                <div className="col-span-1">
                  {i < 3 ? (
                    <div className={`w-8 h-8 rounded-full ${rankColors[i]} flex items-center justify-center text-white text-sm font-bold shadow-sm`}>
                      {i + 1}
                    </div>
                  ) : (
                ...
                <div className="col-span-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                      entry.isCurrentUser ? 'bg-emerald-200' : 'bg-emerald-100'
                    }`}>
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-800 truncate">
                        {entry.full_name}
                        {entry.isCurrentUser && (
                          <span className="ml-2 text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-bold">You</span>
                        )}
                      </p>
                      {entry.city && <p className="text-xs text-slate-400 truncate">{entry.city}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="col-span-2 text-right">
                  <span className="text-sm font-semibold text-slate-700">{entry.latest_footprint}</span>
        ...
        {entries.length > 0 && (
          <div className="mt-4 flex items-center gap-2 justify-center text-xs text-slate-400">
            ...
            <span>Ranked by % improvement in CO₂ emissions, then lowest footprint</span>
          </div>
        )}
      </div>
    </div>
