...
export default function ActionsPage() {
  ...
  const tabs: { key: Tab; label: string; icon: typeof Target; count: number }[] = [
    { key: 'recommended', label: 'Recommended', icon: Target, count: filteredRecommendations.length },
    { key: 'active', label: 'Active Goals', icon: Clock, count: activeGoals.length },
    { key: 'completed', label: 'Completed', icon: CheckCircle, count: completedGoals.length },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-20 pb-12 px-4">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900 mb-1">Actions & Goals</h1>
          <p className="text-sm text-slate-500">
            {tab === 'recommended' && 'Personalized recommendations based on your footprint.'}
          ...
          {tabs.map(t => {
            ...
            return (
              ...
              >
                ...
                {t.label}
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${tab === t.key ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-500'}`}>
                  {t.count}
                </span>
              </button>
            );
        ...
        {tab === 'recommended' && (
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {(['all', 'transport', 'energy', 'food', 'lifestyle'] as FilterCategory[]).map(c => (
              <button
                key={c}
                onClick={() => setCatFilter(c)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  catFilter === c ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
                }`}
              >
                {c.charAt(0).toUpperCase() + c.slice(1)}
              </button>
            ))}
            <span className="text-xs font-medium text-slate-500 uppercase ml-4">Difficulty:</span>
            {(['all', 'easy', 'medium', 'hard'] as FilterDifficulty[]).map(d => (
              <button
                key={d}
                onClick={() => setDiffFilter(d)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  diffFilter === d ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'
                }`}
              >
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {filteredRecommendations.length > 0 ? (
              filteredRecommendations.map((rec, i) => (
                <RecommendationCard key={`${rec.title}-${i}`} recommendation={rec} onAccept={fetchGoals} />
              ))
            ) : (
            ...
            {activeGoals.length > 0 ? (
              ...
              <div className="md:col-span-2">
                <EmptyState
                  ...
                  action={{ label: 'Browse Recommendations', onClick: () => setTab('recommended') }}
                />
              </div>
            )}
          </div>
            ...
            {completedGoals.length > 0 ? (
              ...
              <div className="md:col-span-2">
                <EmptyState
                  ...
                  action={{ label: 'View Active Goals', onClick: () => setTab('active') }}
                />
              </div>
            )}
          </div>
